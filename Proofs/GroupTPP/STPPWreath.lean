...
1. the **STPP capacity inequality** (CKSU `theorem:asi`)
   ...
   Theorem 4.1 (`GroupTPP.CUCapacity.capacity_rpow_le_charDegreeSumReal`); and
...
all of which sit one level below `GroupTPP.CUCapacity` and `GroupTPP.CharDegrees`).
...
* `GroupTPP.STPPWreath.SimultaneousTPP` — the STPP predicate on indexed families
  `A B C : Fin n → Finset G`.
* `GroupTPP.STPPWreath.cyclicGroup` — `C₂ₙ = Multiplicative (ZMod (2n))`, the
  cyclic base group of the Cohn–Umans wreath family.
* `GroupTPP.STPPWreath.ImprimitiveWreathProduct` — the **imprimitive** wreath product
...
* `GroupTPP.STPPWreath.wreathGroup` — `Gₙ = C₂ₙ ≀ Sₙ = C₂ₙⁿ ⋊ Sₙ`, the imprimitive
  Cohn–Umans family (`ImprimitiveWreathProduct (cyclicGroup n) n`), with the
  `Fintype` instance that `GroupTPP.CUCapacity.pseudoExponent` needs.
* `GroupTPP.STPPWreath.wreathGamma` — `γ(Gₙ) = 2 + (1+log 2)/log n`, the leading
  asymptotic bound on the pseudo-exponent.

## Main results (sorry audit: `stpp_capacity_le` and `stpp_capacity_le_of_wreath`)

* `GroupTPP.STPPWreath.SimultaneousTPP.tppR_of` — the STPP entails the per-index
  right-quotient TPP.
* `GroupTPP.STPPWreath.simultaneousTPP_left_iff_comm` — **(`sorry`-free)** abelian
  equivalence with the pre-Df1 (left-quotient per-triple) definition.
* `GroupTPP.STPPWreath.SimultaneousTPP.prod` / `GroupTPP.STPPWreath.SimultaneousTPP.pow`
...
* `GroupTPP.STPPWreath.stpp_capacity_le` — **(`sorry`)** the STPP capacity
  inequality `∑ᵢ (|Aᵢ|·|Bᵢ|·|Cᵢ|)^{ω/3} ≤ D_ω(H)` (CKSU `theorem:asi`);
  the general-`G` form, closed by Pl21 (Clifford campaign) after Df1.
* `GroupTPP.STPPWreath.stpp_capacity_le_of_wreath` — **(`sorry`: multinomial
...
* `GroupTPP.STPPWreath.stpp_capacity_le_comm` — **(`sorry`: via factored skeleton)**
...
* `GroupTPP.STPPWreath.wreath_charDegree_bound` — **(`sorry`-free)** the wreath
...
* `GroupTPP.STPPWreath.stpp_to_tpp_wreath` — **(`sorry`-free)** STPP triples in `H`
  lift to ordinary TPP subsets in `Sₙ ⋉ Hⁿ` (CKSU `theorem:STPP2TPP`).
* `GroupTPP.STPPWreath.wreathH₁`/`wreathH₂`/`wreathH₃`,
  `GroupTPP.STPPWreath.tripleProductProperty_wreathWitness`,
  `GroupTPP.STPPWreath.factorial_pow_three_le_tppCapacity` — **(`sorry`-free)** the
...
* `GroupTPP.STPPWreath.pseudoExponent_wreath_le_gamma` — **(`sorry`-free)** the
  per-`n` pseudo-exponent bound `α(Gₙ) ≤ γ(Gₙ)` (CU wreath theorem).
* `GroupTPP.STPPWreath.pseudoExponent_wreath_tendsto_two` — **(`sorry`-free)** the
  amplification limit `α(Gₙ) → 2` as `n → ∞`.
* `GroupTPP.STPPWreath.abelian_wreath_family_tendsto_two` — **(`sorry`-free)** the
...
namespace GroupTPP.STPPWreath

open GroupTPP.TPP GroupTPP.CUCapacity GroupTPP.CharDegrees
...
* **per-triple TPP (right-quotient):** for each index `i`, the triple
  ...
  `GroupTPP.TPP.TripleProductPropertyR`; and
...
`GroupTPP.TPP.TripleProductProperty` in the per-triple conjunct and the
...
`GroupTPP.TPP.TripleProductProperty.piFinset` (Tp1) by the same componentwise
...
`GroupTPP.TPP.TripleProductPropertyR.prod`; the simultaneous conjunct's
...
`GroupTPP.TPP.TripleProductProperty.piFinset`). An STPP family of `n` triples in
...
The multi-triple analogue of `GroupTPP.CUCapacity.capacity_rpow_le_charDegreeSumReal`
...
(`GroupTPP.CUCapacity.capacity_rpow_le_charDegreeSumReal`). For abelian `G` the
right-hand side collapses to `|G|` (`GroupTPP.CharDegrees.charDegreeSum_two`), the
...
`deriving DecidableEq`.) Needed because `GroupTPP.CUCapacity.pseudoExponent` and
`GroupTPP.CharDegrees.charDegreeSumReal` are stated with `Fintype`. -/
...
theorem wreath_charDegree_bound {H : Type*} [CommGroup H] [Fintype H] [DecidableEq H]
  ...
  have hcle : ∀ c ∈ charDegrees G, c ≤ n.factorial := by
    intro c hc
    rw [← hindex]
    exact GroupTPP.CharDegreesIndexBound.charDegree_le_index_of_comm A hc
  -- Per-entry bound: 1 ≤ c
  have hc1 : ∀ c ∈ charDegrees G, 1 ≤ c := fun c hc =>
    GroupTPP.CharDegreesMul.one_le_of_mem_charDegrees hc
  ...
  -- = (n!)^(ω-1) · |H|^n
  rw [GroupTPP.CharDegreesComm.charDegreeSumReal_of_commGroup]
...
`GroupTPP.TPP.TripleProductPropertyR`; the correct general bridge is
...
Stated in the left-quotient `GroupTPP.TPP.TripleProductProperty` convention. Since
...
of `GroupTPP.CUCapacity` is the base-independent ratio `3·log|G|/log β(G)`, and the
