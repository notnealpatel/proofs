...
2. ~~**The proof of Theorem 4.1.**~~ **Discharged.** The proof (CU.tex:621–666)
   ...
   the character-degree power sum (`GroupTPP.CharDegreesMul`), and the two
...
`GroupTPP.TPP` and `GroupTPP.CharDegrees` and **proved here in full**: the abelian
...
* `GroupTPP.CUCapacity.matrixExponent` — `ω`, the exponent of matrix multiplication.
  Alias for `BilinearComplexity.omega` (discharged foundation).
* `GroupTPP.CUCapacity.pseudoExponent` — `α(G) = 3·log|G| / log β(G)`, the
  Cohn–Umans pseudo-exponent (`theorem:bound` preamble, CU.tex:460–467).
* `GroupTPP.CUCapacity.BetaExceedsD3` — the `D₃` threshold predicate
  `D₃(G) < β(G)` (CU Question `fundamentalq`, CU.tex:774–781). Fully defined:
  both sides are `sorry`-free (`GroupTPP.CharDegrees.charDegrees` landed with the
  `Wd` campaign).

## Main results

* `GroupTPP.CUCapacity.capacity_rpow_le_charDegreeSumReal` — **(proved,
  `sorry`-free)** CU Theorem 4.1 in `β`-form: `β(G)^{ω/3} ≤ D_ω(G)`.
* `GroupTPP.CUCapacity.card_rpow_le_charDegreeSumReal` — **(proved from the `β`-form)**
  the equivalent pseudo-exponent form `|G|^{ω/α(G)} ≤ D_ω(G)`.
* `GroupTPP.CUCapacity.card_rpow_three_div_pseudoExponent` — **(proved,
  `sorry`-free)** the identity `β(G) = |G|^{3/α(G)}` (the bridge between the two
  forms), via `Real.rpow_logb`.
* `GroupTPP.CUCapacity.pseudoExponent_eq_three_of_commGroup` — **(proved,
  `sorry`-free)** `α(G) = 3` for a nontrivial commutative `G` (the abelian
  barrier, via `GroupTPP.TPP.tppCapacity_eq_card`).
* `GroupTPP.CUCapacity.two_lt_pseudoExponent` — **(proved, `sorry`-free)**
  `α(G) > 2` for every nontrivial finite `G` (CU pseudo-exponent lemma,
  CU.tex:478–500).
* `GroupTPP.CUCapacity.betaExceedsD3_certifies_subcubic` — **(proved from Theorem
  4.1, `sorry`-free)** the `D₃` certificate: `D₃(G) < β(G) ⟹ ω < 3` (a single
  group certifies `ω < 3`).
* `GroupTPP.CUCapacity.subcubic_certificate_iff` — **(proved, `sorry`-free)** a
...
namespace GroupTPP.CUCapacity

open GroupTPP.TPP GroupTPP.CharDegrees
...
for downstream compatibility (`GroupTPP.STPPWreath`, the `Dᵣ` sieve). -/
...
(`theorem:bound`) compares against; downstream files (`GroupTPP.STPPWreath`, the
...
`GroupTPP.TPP.tppCapacity_eq_card` (`β(G) = |G|`) with `log|G| ≠ 0` (from `|G| ≥ 2`),
...
Omega}`, `Xlib.TPPProd`, `GroupTPP.CharDegreesMul`) through the per-triple chain:
...
open BilinearComplexity GroupTPP.CharDegreesMul
...
This is the convention adapter between `GroupTPP.TPP.TripleProductProperty`
...
computed from the canonical `GroupTPP.CharDegrees.charDegrees` (the indexed
...
`GroupTPP.CharDegrees.charDegreeSumReal_natCast`), contradicting `D₃(G) < β(G)`;
...
(`GroupTPP.CharDegrees.charDegreeSumReal_natCast`), the goal is exactly
