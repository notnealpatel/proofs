...
This is an abbreviation for the canonical `GroupTPP.TPP.TripleProductPropertyR`,
retained for source compatibility in the dihedral proof. -/
abbrev IsTPP {G : Type*} [Group G] [DecidableEq G] (S T U : Finset G) : Prop :=
  GroupTPP.TPP.TripleProductPropertyR S T U
...
/-! ## Re-homing onto the canonical `GroupTPP.TPP` API

`IsTPP` above is byte-identical to the right-quotient TPP
`GroupTPP.TPP.TripleProductPropertyR`, while the canonical definition is the
left-quotient `GroupTPP.TPP.TripleProductProperty`.  On the *same* ordered
triple the two conventions differ, but they transfer through the inversion
bridge `GroupTPP.TPP.tripleProductProperty_iff_inv`, and `Finset.card_inv`
...
`GroupTPP.TPP.TripleProductPropertyR` (in any group). -/
theorem isTPP_iff {G : Type*} [Group G] [DecidableEq G] (S T U : Finset G) :
    IsTPP S T U ↔ GroupTPP.TPP.TripleProductPropertyR S T U :=
  Iff.rfl
...
property `GroupTPP.TPP.TripleProductProperty` has
...
theorem card_mul_le_of_tripleProductProperty {S T U : Finset (DihedralGroup n)}
    (h : GroupTPP.TPP.TripleProductProperty S T U) :
    3 * (S.card * T.card * U.card) ≤ 8 * n := by
  have hR : IsTPP S⁻¹ T⁻¹ U⁻¹ :=
    (isTPP_iff _ _ _).mpr (GroupTPP.TPP.tripleProductProperty_iff_inv.mp h)
...
theorem three_mul_tppCapacity_le :
    3 * GroupTPP.TPP.tppCapacity (DihedralGroup n) ≤ 8 * n := by
  obtain ⟨⟨S, T, U⟩, hmem, hsup⟩ :=
    Finset.exists_mem_eq_sup (GroupTPP.TPP.tppTriples (DihedralGroup n))
      ⟨(Finset.univ, {1}, {1}), GroupTPP.TPP.mem_tppTriples.mpr GroupTPP.TPP.tpp_trivial⟩
      (fun p => p.1.card * p.2.1.card * p.2.2.card)
  have hcap : GroupTPP.TPP.tppCapacity (DihedralGroup n) = S.card * T.card * U.card := hsup
  rw [hcap]
  exact card_mul_le_of_tripleProductProperty (GroupTPP.TPP.mem_tppTriples.mp hmem)
