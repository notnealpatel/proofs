...
Hilbert–Schmidt bound — is the layer `GroupTPP.FourierBarrier`
(`master_bound`), which sits on top of the sorry-free character-degree
foundation `GroupTPP.CharDegrees` / `GroupTPP.Wedderburn`.  The bridge from
...
`GroupTPP.TPP.card_mul_card_mul_card_le` vs Theorem 3.2). Theorem 3.6 is an
...
* `GroupTPP.BCGPUBarrier.bcgpu_thm_3_2` — the `n(G)` barrier.
* `GroupTPP.BCGPUBarrier.two_le_minNontrivIrrepDim` — nonabelian `G` has `n(G) ≥ 2`.
* `GroupTPP.BCGPUBarrier.bcgpu_cor_3_3` — the `n(G) ≥ |G|^δ` kernel of
  the "cannot meet the packing bound" corollary.
* `GroupTPP.BCGPUBarrier.bcgpu_cor_3_4_kernel` — the Lie-type barrier,
  as a per-group inequality ratio `|S||T||U|/|G|^{3/2} ≤ |G|^{-δ/2} + |G|^{-1/2}`.
* `GroupTPP.BCGPUBarrier.bcgpu_cor_3_5` — the `√2` corollary,
  `|S||T||U| ≤ |G|^{3/2}/√2 + |G|`, for *any* finite group.
* `GroupTPP.BCGPUBarrier.bcgpu_thm_3_6` — the normalizer barrier.
* `GroupTPP.BCGPUBarrier.bcgpu_cor_3_8` — the center barrier.
...
namespace GroupTPP.BCGPUBarrier
...
number. This is `GroupTPP.CharDegrees.minNontrivIrrepDim G` cast to `ℝ`; it equals
...
commutative (`GroupTPP.FourierBarrier.exists_one_lt_dim_of_nonabelian`), so some
...
theorem two_le_minNontrivIrrepDim (hG : ∃ a b : G, a * b ≠ b * a) :
  ...
  obtain ⟨i₁, hi₁⟩ := GroupTPP.FourierBarrier.exists_one_lt_dim_of_nonabelian e₀ hG
...
by the unitarian trick (`GroupTPP.FourierBarrier.exists_isUnitary`), and identify
...
`GroupTPP.FourierBarrier.master_bound`, applied with `n := minNontrivIrrepDim G`
...
theorem bcgpu_thm_3_2 {S T U : Finset G} (hG : ∃ a b : G, a * b ≠ b * a)
  ...
  obtain ⟨k, d, hd, ⟨e₀⟩⟩ :=
    IsSemisimpleRing.exists_algEquiv_pi_matrix_of_isAlgClosed ℂ (MonoidAlgebra ℂ G)
  haveI := hd
  obtain ⟨e, he⟩ := GroupTPP.FourierBarrier.exists_isUnitary e₀
  ...
  exact GroupTPP.FourierBarrier.master_bound he (two_le_minNontrivIrrepDim hG) hnd
    h hS hT hU
...
`GroupTPP.CharDegrees.card_charDegrees`.)
...
(`GroupTPP.TPP.card_mul_card_mul_card_le`). Otherwise `n(G) ≥ 2`
...
barrier `GroupTPP.TPP.card_mul_card_mul_card_le`,
`∏ᵢ |Hᵢ ∩ Z(G)| ≤ |Z(G)|`. -/
