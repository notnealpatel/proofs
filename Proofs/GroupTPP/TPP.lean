...
* `GroupTPP.TPP.TripleProductProperty` — the TPP predicate on `S T U : Finset G`.
* `GroupTPP.TPP.SubgroupTripleProductProperty` — the TPP on subgroup carriers.
* `GroupTPP.TPP.tppCapacity` — `β(G)`, the sup of `|S| · |T| · |U|` over TPP triples.
* `GroupTPP.TPP.stppCapacity` — `β₀(G)`, the same sup over subgroup triples.
* `GroupTPP.TPP.rho0` — `ρ₀(G) = β₀(G) / |G|`, the subgroup-TPP ratio, as a real.
* `GroupTPP.TPP.TripleProductPropertyR` — the *right-quotient* TPP
  (`Q(X) = X * X⁻¹`, the Neumann/Hedtke–Murthy convention).

## Main results

* `GroupTPP.TPP.TripleProductProperty.injOn_mul` — under the TPP in a *commutative*
  group, `(s, t, u) ↦ s * t * u` is injective on `S ×ˢ T ×ˢ U`.
* `GroupTPP.TPP.card_mul_card_mul_card_le` — the abelian barrier,
  `|S| · |T| · |U| ≤ |G|`.
* `GroupTPP.TPP.tppCapacity_eq_card` — `β(G) = |G|` for commutative `G`.
* `GroupTPP.TPP.card_le_tppCapacity` — the trivial lower bound `|G| ≤ β(G)`.
* `GroupTPP.TPP.stppCapacity_le_tppCapacity` — `β₀(G) ≤ β(G)`.
* `GroupTPP.TPP.card_inter_ST_le_one` (and `…_TU`, `…_SU`) — Hedtke's pairwise
  intersection bound: two of `S, T, U` meet in ≤ 1 point (third nonempty).
* `GroupTPP.TPP.card_add_card_add_card_le` — Hedtke's Corollary 6,
  `|S| + |T| + |U| ≤ |G| + 2` for a TPP triple of nonempty subsets.
* `GroupTPP.TPP.tripleProductPropertyR_iff_inv` — the inversion bridge between the
...
theorem TripleProductPropertyR.le_tppCapacity {S T U : Finset G}
  ...
  have hb := GroupTPP.TPP.le_tppCapacity (tripleProductPropertyR_iff_inv.mp h)
