...
`GroupTPP.BCGPUBarrier` instantiates `n := minNontrivIrrepDim G` through the
`GroupTPP.CharDegrees` bridge to obtain BCGPU Theorem 3.2 and its corollaries.
-/

open scoped BigOperators
open scoped Matrix

namespace GroupTPP.FourierBarrier
