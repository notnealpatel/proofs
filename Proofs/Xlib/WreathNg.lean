...
`n₂` itself is `Xlib.CharDegrees.minNontrivIrrepDim`, which rests on the single
foundational `sorry` of `Xlib.CharDegrees` (the *indexed* Wedderburn–Artin
...
(the same convention as `Xlib.STPPWreath.permArrowHom`). We rebuild it here so the
...
`Xlib.CharDegrees.minNontrivIrrepDim` is `n₂(G)`, defined as the minimum of the
...
theorem minNontrivIrrepDim_wreathS2_eq_two_of_ne_zero
    ...
    (hne : Xlib.CharDegrees.minNontrivIrrepDim (Wreath G 2) ≠ 0)
    (hle : Xlib.CharDegrees.minNontrivIrrepDim (Wreath G 2) ≤ 2) :
    Xlib.CharDegrees.minNontrivIrrepDim (Wreath G 2) = 2 := by
