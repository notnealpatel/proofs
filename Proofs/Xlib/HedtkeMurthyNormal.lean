...
(`Xlib.TPP.card_mul_card_mul_card_le`): once one factor is normal, the product
...
> `Xlib.TPP.SubgroupTPP.injOn_quot` — the map `↥T × ↥U → G ⧸ S`,
...
* `Xlib.TPP.SubgroupTPP.injOn_quot` — the normal-quotient injection.
* `Xlib.TPP.SubgroupTPP.card_mul_card_le_index_of_normal_left` — `|T|·|U| ≤ [G:S]`.
* `Xlib.TPP.card_mul_card_mul_card_le_of_normal` — Theorem 3.5 in `Nat.card`
  form: a normal member (in any of the three slots) forces the product bound.
* `Xlib.TPP.card_mul_card_mul_card_le_toFinset_of_normal` — the same bound in the
  `Finset`-carrier phrasing that mirrors `Xlib.TPP.card_mul_card_mul_card_le`.
...
slot. This is the nonabelian companion to `Xlib.TPP.card_mul_card_mul_card_le`:
...
`Xlib.TPP.card_mul_card_mul_card_le`. -/
