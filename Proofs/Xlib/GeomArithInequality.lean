...
capacity inequality (CKSU `theorem:asi`, `Xlib.STPPWreath.stpp_capacity_le`).
...
* `Xlib.GeomArithInequality.le_of_pow_le_poly_mul_pow` — the **polynomial
  prefactor absorption** limit engine: `x ^ N ≤ K * (N+1)^k * y ^ N` for all
  `N ≥ 1` forces `x ≤ y`. Generalizes
  `Xlib.CUCapacity.le_of_pow_le_const_mul_pow` (the `k = 0` case).
* `Xlib.GeomArithInequality.sum_le_of_multinomial_prod_pow_le` — CKSU
  `lemma:geom->arith` over any finite index type, without the (unneeded)
  nonnegativity of `s`.
* `Xlib.GeomArithInequality.geom_arith_inequality` — the paper-literal
...
namespace Xlib.GeomArithInequality

/-- **Polynomial-prefactor absorption** (the `N → ∞` limit engine): if
`x ^ N ≤ K * (N + 1) ^ k * y ^ N` for all `N ≥ 1` with `y ≥ 0`, then `x ≤ y`.

This generalizes `Xlib.CUCapacity.le_of_pow_le_const_mul_pow` (which is the
