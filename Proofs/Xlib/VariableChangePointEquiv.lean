...
same-curve only.  `Xlib.ShortCurveScaling.scaleEquiv` covers the
...
namespace Xlib.VariableChangePointEquiv
...
does not provide; `Xlib.ShortCurveScaling.scaleEquiv` is the
`r = s = t = 0` case. -/
...
open Xlib.ShearAdditionEC (shortCurve)
open Xlib.ShortCurveScaling (shortCurve_a₄ shortCurve_a₆)
...
isomorphism `Xlib.ShortCurveScaling.scaleEquiv`
...
end Xlib.VariableChangePointEquiv
