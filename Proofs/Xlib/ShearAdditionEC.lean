...
`Xlib.ShearAddition` studies the "remaining transformation" of affine
...
* `singleShear_computes_add` — the concrete one-shear circuit
  `Xlib.ShearAddition.singleShear`, fed `(c, λ, 0)` with `c = a - W.addX …`,
...
namespace Xlib.ShearAdditionEC
...
concrete single-shear realisation `Xlib.ShearAddition.singleShear` the chord
...
theorem singleShear_computes_add (h1 : W.a₁ = 0) (h3 : W.a₃ = 0) (ℓ : F) :
  ...
  rw [Xlib.ShearAddition.singleShear_zero]
...
map whose shear complexity `Xlib.ShearAddition` analyses. -/
...
`Xlib.ShearAddition` — so every reversibility/lower-bound statement about
