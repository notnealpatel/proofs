...
* `Xlib.HedtkeBasic.tripleProductProperty_iff_leftQuot` and
  `Xlib.HedtkeBasic.IsBasic.tpp_iff` — **Hedtke–Murthy Theorem 3.1**
...
* `Xlib.HedtkeBasic.subgroupTripleProductProperty_iff` — the subgroup
...
(`Xlib.TPP.leftQuot`, `Q(X) = {x'⁻¹ x}`), the characterization keeps exactly
the same shape:
