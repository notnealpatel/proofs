...
* `Xlib.GroupAlgebraCenter.classSum` — the class sum of a conjugacy class in `k[G]`.
* `Xlib.GroupAlgebraCenter.classSumBasis` — the class sums as a
  `Basis (ConjClasses G) k (Subalgebra.center k (MonoidAlgebra k G))`.

## Main results

* `Xlib.GroupAlgebraCenter.mem_center_iff` — an element of `k[G]` is central iff
  its coefficient function is constant on conjugacy classes (no finiteness needed).
* `Xlib.GroupAlgebraCenter.finrank_center` — `finrank k Z(k[G]) = #ConjClasses G`
...
`ℂ`) is `Xlib.CharDegrees.card_charDegrees`.
-/

open Module

namespace Xlib.GroupAlgebraCenter
