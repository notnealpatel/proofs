...
For subgroup carriers, the Cohn–Umans TPP (general subset form, as in `Xlib.TPP`) is
...
Reduces to `Xlib.TPP.tppCapacity_eq_card` via a local `CommGroup` instance. -/
