...
of the TPP (`Xlib.TPP.IsTPPTriple`) were developed in discussion (2026-07).
The statement was verified computationally before formalization (see
`Scratch/verify_tpp.sage`).

## Main definitions

* `Xlib.MonomialRealization.IsMonomialRealization` -- the coefficient-level
  identity that a monomial realization satisfies.
* `Xlib.MonomialRealization.IsSemanticMonomialRealization` -- the semantic
  identity: `trace(A*B*C) = Phi(phiA(A), phiB(B), phiC(C))` for all matrices.
* `Xlib.MonomialRealization.coeffOne` -- the coefficient of `1_G` in a group
  algebra element.
* `Xlib.MonomialRealization.Phi` -- the group trilinear form.
* `Xlib.MonomialRealization.phiA`, `phiB`, `phiC` -- monomial embeddings.

## Main results

* `Xlib.MonomialRealization.IsSemanticMonomialRealization.toMonomialRealization`
  -- the semantic identity implies the coefficient-level identity.
* `Xlib.MonomialRealization.no_cancellation` -- **Claim 1**: a monomial
  realization forces `a(i,j) * b(j',k) * c(k',i') = 1 -> j = j' /\ k = k' /\ i = i'`.
* `Xlib.MonomialRealization.diagonal_coeff` -- the diagonal coefficient
  identity: `alpha(i,j) * beta(j,k) * gamma(k,i) = 1`.
* `Xlib.MonomialRealization.diagonal_group_prod_eq_one` -- the diagonal
  group products are always `1`.
* `Xlib.MonomialRealization.collision_preimages_diagonal` -- **Claim 2**:
  the set-level collision form, connecting to preimage consistency.
* `Xlib.MonomialRealization.prod` -- **Claim 3**: the Kronecker product of
...
open Xlib.TPP Finset Matrix MonoidAlgebra
