...
`Xlib.ShearAddition` treats the single-shear (`s = 1`, degree-2) slice of this
...
degree `D` needs at least `log₂ D` shears (`Xlib.ShearInversionLB` instantiates
