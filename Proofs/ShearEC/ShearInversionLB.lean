...
Flagship lower bound for the shear-circuit model of `ShearEC.ShearCircuit`.
...
(`ShearEC.TotalDegreeAeval.natDegree_aeval_le`). Combining:
...
* `secp256k1_inversion_needs_256_shears` — for the secp256k1 base field
  ...
  Pratt/Lucas certificate of `ShearEC.Secp256k1Prime` (see `secp256k1P_prime`
...
affine group law is a bounded number of shears (`ShearEC.ShearAddition`).
...
namespace ShearEC.ShearInversionLB

open ShearEC.ShearCircuit ShearEC.ShearCircuit.Circuit
...
theorem card_sub_two_le_two_pow_shearCount {n : ℕ} (C : Circuit n K)
  ...
  have hdeg : g.natDegree ≤ 2 ^ shearCount C := by
    have h1 : g.natDegree ≤ (polys C out).totalDegree * 1 := by
      refine ShearEC.TotalDegreeAeval.natDegree_aeval_le _ _ fun i => ?_
      by_cases h : i = inp <;> simp [h]
    rw [mul_one] at h1
    exact h1.trans (totalDegree_polys_le C out)
  have hg : ∀ x : K, g.eval x = x⁻¹ := by
    intro x
    rw [hgdef, ShearEC.TotalDegreeAeval.eval_aeval]
