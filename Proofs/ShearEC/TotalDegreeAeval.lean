...
* `ShearEC.TotalDegreeAeval.totalDegree_aeval_le` — **the gap**: if every `f i`
  ...
  targets.
* `ShearEC.TotalDegreeAeval.totalDegree_aeval_le_sup_vars` — the sharp form:
...
* `ShearEC.TotalDegreeAeval.totalDegree_aeval_le_sup` — corollary over a
  `Fintype` variable set with `d = ⊔ i, totalDegree (f i)`.
* `ShearEC.TotalDegreeAeval.natDegree_aeval_le` — the one-variable-target bound,
...
* `ShearEC.TotalDegreeAeval.eval_aeval` — evaluation commutes with the
...
(`ShearEC.ShearCircuit`) and the reversible-inversion lower bound
(`ShearEC.ShearInversionLB`): a composition of `s` multiply-add gates has
coordinate degree `≤ 2 ^ s`, so a target of algebraic degree `D` needs
`≥ log₂ D` gates.
-/

namespace ShearEC.TotalDegreeAeval
