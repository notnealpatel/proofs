...
* `ShearEC.ShearAddition.shear` — the primitive `M` as an `Equiv` (it is reversible,
  with inverse `w ↦ w - u·v`).
* `ShearEC.ShearAddition.singleShear` — the concrete one-shear realisation
...
* `ShearEC.ShearAddition.no_reversible_clean_shear_addition` — **the impossibility
...
* `ShearEC.ShearAddition.ancilla_carries_lambda` — **the invariant**: any injective
...
* `ShearEC.ShearAddition.shear_comp_normal_form` — **the rank ≤ 1 normal form**: any
...
* `ShearEC.ShearAddition.shearComp` / `shearComp_bijective_and_normal_form` — **payload
...
`ShearEC.ShearAdditionEC` proves that on any Weierstrass curve with
...
`ShearEC.ShearCircuit` generalises the one-shear model to circuits of `s` shears
(degree `≤ 2^s`), and `ShearEC.ShearInversionLB` derives the flagship consequence:
...
namespace ShearEC.ShearAddition
...
(`ShearEC.ShearQuadraticRank.not_isSumOfProducts_one_quad` /
...
quadratic part `c·λ` has product-rank 1; see `ShearEC.ShearQuadraticRank` for a
