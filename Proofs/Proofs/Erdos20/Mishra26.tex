...
\begin{document}
	%
	\title{Erd\H{o}s Rado Sunflower (Conjecture) Theorem}
	%
	...
	\begin{proof}
		Let $\mathcal{F}^* = \mathcal{F}_{n-1,n}$ be the fully shifted (stable) family. Let $\mathcal{S}^* = \{ S_1, \dots, S_m \}$ be a maximum sunflower in $\mathcal{F}^*$ of size $m = \tau(\mathcal{F}^*)$. Let $K$ be the kernel of $\mathcal{S}^*$.
		
		Case 1: $1 \notin K$~ If $1 \notin K$, then at most one set in $\mathcal{S}^*$ contains $1$. The subfamily $\mathcal{S}' = \{ S \in \mathcal{S}^* : 1 \notin S \}$ has size at least $m-1$. For any $S \in \mathcal{S}'$, let $p$ be an element in its petal ($p \in S \setminus K$). Since $\mathcal{F}^*$ is stable, the set $T = (S \setminus \{p\}) \cup \{1\}$ must also be in $\mathcal{F}^*$. The collection $\{ T_S \}_{S \in \mathcal{S}'}$ forms a family where each set contains $1$, derived from distinct sources $p$. As we will prove in Case 2, the number of such distinct sources is bounded by $3\tau(\mathcal{F})$.Thus, $m-1 \leq 3\tau(\mathcal{F})$, which implies $\tau(\mathcal{F}^*) \leq 3\tau(\mathcal{F}) + 1 \leq 3\tau(\mathcal{F})^2$.
		
		Case 2: $1 \in K$ ~ Every set $S \in \mathcal{S}^*$ contains $1$. For each $S \in \mathcal{S}^*$, there is a unique source element $x$ such that $S$ was derived from an original set $F \in \mathcal{F}$ by shifting $x \to 1$.
		$$S = (F \setminus \{x\}) \cup \{1\} \quad \text{and} \quad F = (S \setminus \{1\}) \cup \{x\}.$$
		We partition $\mathcal{S}^*$ based on these sources. Let $X$ be the set of all sources. 
		For each $x \in X$, define the subfamily:
		$$\mathcal{S}^*_x = \{ S \in \mathcal{S} \mid S \text{ was derived from some } F \in \mathcal{F} \text{ by shifting } x \to 1 \}$$
		\begin{align}\label{eq:todo}
			\tau(\mathcal{F}^*) = \sum_{x \in X} |\mathcal{S}^*_x|.
		\end{align}
		
		
		To establish the lemma, we proceed in two steps.
		\begin{itemize}
			\item Step A: We bound the size of each part: $|\mathcal{S}^*_x| \leq \tau(\mathcal{F})$.(The pre-images of $\mathcal{S}^*_x$ all contain $x$, forming a sunflower in $\mathcal{F}$).
			\item Step B: We bound the number of sources: $|X| \leq 3\tau(\mathcal{F})$.
		\end{itemize}
		
		
		{\noindent \bf Step A:} ~ Let $\mathcal{G}_x$ be the pre-images of $\mathcal{S}^*_x$ in $\mathcal{F}$. For any $S_a, S_b \in \mathcal{S}^*_x$ with pre-images $F_a, F_b$, we have $F_a = (S_a \setminus \{1\}) \cup \{x\}$ and $F_b = (S_b \setminus \{1\}) \cup \{x\}$. The intersection is $F_a \cap F_b = ((S_a \cap S_b) \setminus \{1\}) \cup \{x\} = (K \setminus \{1\}) \cup \{x\}$. Since this intersection is constant, $\mathcal{G}_x$ is a sunflower in $\mathcal{F}$. Thus, $|\mathcal{S}_x| = |\mathcal{G}_x| \leq \tau(\mathcal{F})$.
		
		{\noindent \bf Step B:} ~ 
		For each $x \in X$, let $F_x \in \mathcal{F}$ be a representative pre-image such that shifting $x \to 1$ transforms $F_x$ into a set $S_x \in \mathcal{S}^*$.
		Precisely, $S_x = (F_x \setminus \{x\}) \cup \{1\}$.
		
		We construct a directed graph $H$ as follows:
		\begin{itemize}
			\item For each $x \in X$, add a vertex $v_x$ corresponding to the triple $(F_x,S_x,x)$ : $|V(H)|=|X|$.
			\item  Add a directed edge from $v_x=(F_x,S_x,x)$ to $v_y=(F_y,S_y,y)$ if
			$x \in S_y$.
		\end{itemize}
		
		Since $S_y \setminus K$ are disjoint petals, each $x$ can belong to at most one $S_y$. Thus, {\bf the graph $H$ has max out-degree 1}.
		
		Let $I \subseteq X$ be such that $\{v_i\}_{i\in I}$ form an independent set in $H$. This means that for any $x,y \in I$
		$x \not\in S_y$ and $y \not\in S_x$.
		Observe that for any $x,y \in I$, 
		$F_x \cap F_y =K \setminus \{1\}$: $$F_x \cap F_y = \left( (S_x \setminus \{1\}) \cup \{x\} \right) \cap \left( (S_y \setminus \{1\}) \cup \{y\} \right)=(S_x \cap S_y)\setminus \{1\}.$$
		Therefore, $\{F_x\}_{x \in I}$ is a sunflower in $\mathcal{F}$. Therefore, size of the maximum independent set in $H$, $\alpha(H)$, is at most $\tau(\mathcal{F})$:
		\begin{align}\label{eq:imp}
			\alpha(H) \le \tau(\mathcal{F}).
		\end{align}
		...
		\end{claim}
		\begin{proof}
			To prove the claim, we prove that each connected component of $H$ is 3-colorable. Let the underlying undirected graph of $H$ be $G$.
			
			Every directed edge in $H$ corresponds to exactly one undirected edge in $G$. In any subgraph of $G$, the number of edges is equal to the sum of the out-degrees of the vertices in that subgraph (with respect to $H$). Because the maximum out-degree of every vertex in $H$ is at most 1, the number of edges in any weakly connected component $C_i$ (which is a subgraph of $G$) can never exceed its number of vertices. If a component has $n_i$ vertices and $t_i$ edges, we strictly have:$$t_i \le n_i.$$
			Further, since $C_i$ is connected, $$t_i \ge n_i-1.$$
			
			Combining these, every connected component $C_i$ in $G$ must fall into one of two strict categories:
			\begin{enumerate}
				\item $t_i = n_i - 1$: The component is a tree, which is 2-colorable. 
				\item $t_i = n_i$: The component is a pseudo-tree (contains exactly 1 cycle). This is $3$-colorable.
			\end{enumerate}
		\end{proof}
		
		Since, for any graph $G$, $\chi(G)\alpha(G) \geq |V(G)|$,
		it follows that $\alpha(H) \geq \frac{|V(H)|}{3}=\frac{|X|}{3}$. Combining this with Inequality \ref{eq:imp}, we get $|X| \leq 3\tau(\mathcal{F})$, as desired.
		Now we are ready to prove the lemma. 
		From Equation \ref{eq:todo}, we get:
		$$\tau(\mathcal{F}^*) = \sum_{x \in X}|\mathcal{S}^*_x| \leq |X| \cdot \tau(\mathcal{F}) \leq 3\tau(\mathcal{F})^2.$$
	\end{proof}
	
	

\section{Sunflower Number, 1-Compressed families and Shadows}\label{sec:ss}
