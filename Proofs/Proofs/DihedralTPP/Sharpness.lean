...
private lemma tripleProductProperty_S6_T6_U6 :
    Xlib.TPP.TripleProductProperty S6 T6 U6 := by
  decide
...
theorem exists_tripleProductProperty_bound_eq :
    ∃ S T U : Finset (DihedralGroup 6),
      Xlib.TPP.TripleProductProperty S T U ∧
...
strictly beating the abelian barrier `Xlib.TPP.tppCapacity_eq_card`. -/
theorem tppCapacity_dihedralGroup_six :
    Xlib.TPP.tppCapacity (DihedralGroup 6) = 16 := by
  have hlow : 16 ≤ Xlib.TPP.tppCapacity (DihedralGroup 6) := by
    have h := Xlib.TPP.le_tppCapacity tripleProductProperty_S6_T6_U6
    have hc : S6.card * T6.card * U6.card = 16 := by decide
    rwa [hc] at h
  have hhigh : 3 * Xlib.TPP.tppCapacity (DihedralGroup 6) ≤ 8 * 6 :=
