...
# This file was *autogenerated* from the file /workspace/Bm-m3-enumerate.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0p5 = RealNumber('0.5'); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_16 = Integer(16); _sage_const_12 = Integer(12)# =============================================================================
...
def _pyint(x):
    ...
    f = float(x)
    return builtins.int(f + (_sage_const_0p5  if f >= _sage_const_0  else -_sage_const_0p5 ))
...
def simultaneous_eigenspaces(ms):
    n = ms[_sage_const_0 ].dimensions()[_sage_const_0 ]
...
def weight_decomposition(a, C):
    ...
    eig.sort(key=lambda p: tk(p[_sage_const_1 ]))
    M = {wt: V for V, wt in eig}
    tot = [(wt, V.dimensions()[_sage_const_1 ]) for V, wt in eig]
    wtg = DiGraph()
    for p in tot:
        wt, mult = p; B = M[wt]
        for i, x in enumerate(a[_sage_const_0 ]):
            ...
            if wt2 in M:
                wtg.add_edge(p, (wt2, M[wt2].dimensions()[_sage_const_1 ]), _restrict_map(x, M[wt2], B))
...
def build_Mn_tensor(n):
    ...
    for i in range(n):
        for j in range(n):
            a = i * n + j
            for k in range(n):
                T[a][(j * n + k, k * n + i)] = QQ(_sage_const_1 )
...
def build_Tperp_and_module(T, reps, C):
    ...
    rAB = module_product(reps[_sage_const_0 ], reps[_sage_const_1 ])
...
def test_210_120(E110, adim, r):
    ...
    R = E110.base_ring()
    dom = E110.dimensions()[_sage_const_1 ] * adim     # 16*9 = 144
    need = dom - r                        # 128; pass iff rank <= need iff (need+1)-minors vanish
    M210 = skew_210_map(E110, adim)
    g210 = rankdrop_minors(M210, need + _sage_const_1 )
    one = (R.one() if R is not QQ else _sage_const_1 )
    drops210_everywhere = (g210 == [])
    never210 = (g210 == [one] or g210 == [_sage_const_1 ])
    if never210:
        return ('fail', None)
    M120 = skew_210_map(transpose_tensor(E110, adim), adim)
    g120 = rankdrop_minors(M120, need + _sage_const_1 )
    never120 = (g120 == [one] or g120 == [_sage_const_1 ])
    if never120:
        return ('fail', None)
...
def test_210_120_fast(E110p_AB, adim, r, ctx):
    ...
    def side(E110p, coker, crank):
        ...
        return rankdrop_minors(P, need_res + _sage_const_1 )    # 48-minors

    g210 = side(E110p_AB, ctx['coker210'], ctx['crank210'])
    if g210 == [one] or g210 == [_sage_const_1 ]:
        return ('fail', None)
    drops210 = (g210 == [])
    ...
    if g120 == [one] or g120 == [_sage_const_1 ]:
        return ('fail', None)
    drops120 = (g120 == [])
...
# 4. Driver
# -----------------------------------------------------------------------------
def run(n=_sage_const_3 , r=_sage_const_16 , upset_filter=None, chart_budget=None, verbose=True):
    T, reps, C = build_Mn_tensor(n)
    mdata, em = build_Tperp_and_module(T, reps, C)
    adim = n**_sage_const_2 
    ...
    survivors = []
    stats = {'n': n, 'r': r, 'num_upsets': len(ups), 'charts_total': _sage_const_0 ,
             'charts_closure_rejected': _sage_const_0 , 'charts_fail': _sage_const_0 ,
             'charts_pass_all': _sage_const_0 , 'charts_pass_locus': _sage_const_0 }
    idxs = range(len(ups)) if upset_filter is None else upset_filter
    for ui in idxs:
        up = ups[ui]
        desc = [([_pyint(x) for x in p[_sage_const_0 ]], builtins.int(p[_sage_const_1 ]), builtins.int(m)) for p, m in up]
        ncharts = prod(binomial(p[_sage_const_1 ], m) for p, m in up)
        t0 = time.time(); cc = _sage_const_0 
        for nzs, nvars in charts_for_upset(up):
            if chart_budget is not None and cc >= chart_budget: break
            cc += _sage_const_1 ; stats['charts_total'] += _sage_const_1 
            res = hwv_for_chart(mdata, up, nzs, nvars)
            if res is None:
                stats['charts_closure_rejected'] += _sage_const_1 ; continue
            ...
            if verdict == 'fail':
                stats['charts_fail'] += _sage_const_1 
            elif verdict == 'pass_all':
                stats['charts_pass_all'] += _sage_const_1 
                ...
                if verbose: print(json.dumps(survivors[-_sage_const_1 ])); sys.stdout.flush()
            else:
                stats['charts_pass_locus'] += _sage_const_1 
                survivors.append({'upset': builtins.int(ui), 'nzs': [[builtins.int(x) for x in z] for z in nzs],
        ...
        if verbose:
            print(json.dumps({'_upset_done': builtins.int(ui), 'desc': desc,
                              ...
                              'time_s': builtins.round(float(time.time() - t0), _sage_const_1 )})); sys.stdout.flush()
...
# =============================================================================
UPSET_FILTER = [_sage_const_0 ]         # e.g. [0, 1, 2] ; None = all
CHART_BUDGET = _sage_const_12           # e.g. 5000 ; None = unlimited
USE_FAST = False

if __name__ == '__main__' or True:
    if USE_FAST:
        os.environ['BM5_FAST'] = '1'
    survivors, stats = run(n=_sage_const_3 , r=_sage_const_16 , upset_filter=UPSET_FILTER,
