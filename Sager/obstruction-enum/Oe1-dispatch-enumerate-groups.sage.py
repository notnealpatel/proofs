...
# This file was *autogenerated* from the file /workspace/Oe1-dispatch-enumerate-groups.sage
from sage.all_cmdline import *   # import sage library

_sage_const_168 = Integer(168); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_3 = Integer(3); _sage_const_22 = Integer(22)# Oe1 DISPATCH (medium, single-core): EXHAUSTIVE enumeration of finite groups
...
import json, math
ORDER_BOUND = _sage_const_168    # covers A_4(12),S_4(24),A_5(60),PSL(2,7)(168). Raise to 1080
...
def min_orbit_lb(G, order):
    ...
    try:
        maxs=gap.MaximalSubgroupClassReps(G); Mmax=_sage_const_1 
        for t in range(_sage_const_1 ,Integer(gap.Length(maxs))+_sage_const_1 ):
            mo=Integer(gap.Order(maxs[t]))
            if mo>Mmax: Mmax=mo
        return order//Mmax if Mmax>_sage_const_0  else order
    except Exception:
        return order

candidates=[]
for order in range(_sage_const_3 , ORDER_BOUND+_sage_const_1 ):
    if order % _sage_const_3  != _sage_const_0 :
    ...
    try:
        ...
        continue
    for gi in range(_sage_const_1 , ngroups+_sage_const_1 ):
        try:
            G=gap.SmallGroup(order, gi)
            ct=gap.CharacterTable(G)
            irr=gap.Irr(ct)
            # degrees
            degs=[Integer(gap.Degree(irr[t])) for t in range(_sage_const_1 ,Integer(gap.Length(irr))+_sage_const_1 )]
            if _sage_const_3  not in degs:
                continue
            # check there is an order-3 element
            ords=[Integer(x) for x in gap.OrdersClassRepresentatives(ct)]
            if _sage_const_3  not in ords:
                continue
            # check FAITHFUL 3-dim irrep exists: kernel trivial
            faithful3=False
            for t in range(_sage_const_1 ,Integer(gap.Length(irr))+_sage_const_1 ):
                if Integer(gap.Degree(irr[t]))!=_sage_const_3 : continue
                ker=gap.KernelOfCharacter(irr[t])
                if Integer(gap.Order(ker))==_sage_const_1 :
                    faithful3=True; break
            if not faithful3:
                continue
            name=str(gap.StructureDescription(G))
            mlb=min_orbit_lb(G, order)
            rank_lb=_sage_const_1 +mlb
            can_fit=rank_lb<=_sage_const_22 
