...
# This file was *autogenerated* from the file /workspace/Oe1-family2-A5-full.sage
from sage.all_cmdline import *   # import sage library

_sage_const_5 = Integer(5); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_3 = Integer(3); _sage_const_2000 = Integer(2000); _sage_const_60 = Integer(60); _sage_const_9 = Integer(9)# Oe1 Family 2: full A_5 (icosahedral) GM-ansatz analysis for <3,3,3>.
...
assert len(ELEM)==_sage_const_60 , len(ELEM)
...
def antisym_basis():
    out=[]
    for (i,j) in [(_sage_const_0 ,_sage_const_1 ),(_sage_const_0 ,_sage_const_2 ),(_sage_const_1 ,_sage_const_2 )]:
        E=matrix(K,_sage_const_3 ,_sage_const_3 ); E[i,j]=_sage_const_1 ; E[j,i]=-_sage_const_1 ; out.append(E)
    return out
AB=antisym_basis()
...
def antisym_to_vec(X):  # X antisymmetric -> (X01,X02,X12)
    return vector(K,[X[_sage_const_0 ,_sage_const_1 ],X[_sage_const_0 ,_sage_const_2 ],X[_sage_const_1 ,_sage_const_2 ]])
def conj_on_antisym(g):
    M=matrix(K,_sage_const_3 ,_sage_const_3 )
    for col,B in enumerate(AB):
        img = g*B*g.inverse()
        v=antisym_to_vec(img)
        for r in range(_sage_const_3 ): M[r,col]=v[r]
    return M
# End dim on antisym:
rows2=[]; I3b=identity_matrix(K,_sage_const_3 )
for g in ELEM:
    A=conj_on_antisym(g)
    rows2.append(A.tensor_product(A.inverse().transpose())-identity_matrix(K,_sage_const_9 ))
big2=block_matrix(K,[[m] for m in rows2])
print(f"dim End on antisymmetric (Lambda^2) subspace = {_sage_const_9 -big2.rank()} (1 => Lambda^2 irreducible 3-dim ~ rho)")
...
for i in range(_sage_const_3 ):
    for j in range(_sage_const_3 ):
        E=matrix(K,_sage_const_3 ,_sage_const_3 ); E[i,j]=_sage_const_1 ; Ebasis.append(E)
...
for g in ELEM:
    ...
    if key not in seen and len(H)>_sage_const_1 :
...
for o in sorted(byorder.keys()):
    H=byorder[o][_sage_const_0 ]
    fb=fixedBasis_sub(H)
    orbit=_sage_const_60 //o
    print(f"  |H|={o:2d}  orbit={orbit:2d} rank={_sage_const_1 +orbit:2d}  fixed_dim(M3)={len(fb)}  (#subgroups this order: {len(byorder[o])})")
