...
/-!
# Exact doubling for two-bit addition chains: solution

The independent addition-chain model below is repeated from the challenge;
this file does not import the challenge. The bridge proves equivalence of the
two inductive chain predicates and equality of the sets of attainable chain
lengths before applying the repository's bounded-family doubling theorem.
Neither archived open conjecture is used.
-/
...
private theorem isAddChain_iff_source (c : List ℕ) :
    IsAddChain c ↔ NumberComplexity.IsAddChain c := by
  constructor
  · intro hc
    induction hc with
    | one => exact .one
    | add ha hb _ ih => exact .add ha hb ih
  · intro hc
    induction hc with
    | one => exact .one
    | add ha hb _ ih => exact .add ha hb ih

private theorem l_eq_source (n : ℕ) : l n = NumberComplexity.l n := by
  change sInf (Set.range (fun c : AdditionChain n => chainSteps c.val)) =
    sInf (Set.range (fun c : NumberComplexity.AdditionChain n =>
      NumberComplexity.chainSteps c.val))
  congr 1
  ext r
  constructor
  · rintro ⟨c, hr⟩
    refine ⟨⟨c.val, (isAddChain_iff_source c.val).mp c.property.1, c.property.2⟩, ?_⟩
    exact hr
  · rintro ⟨c, hr⟩
    refine ⟨⟨c.val, (isAddChain_iff_source c.val).mpr c.property.1, c.property.2⟩, ?_⟩
    exact hr
...
theorem l_two_mul_eq_add_one_of_binaryWeight_le_two {k : ℕ} (hk : 0 < k)
  ...
  rw [l_eq_source, l_eq_source]
  exact NumberComplexity.l_two_mul_eq_add_one_of_binaryWeight_le_two hk hv
...
#print axioms isAddChain_iff_source
#print axioms l_eq_source
#print axioms NumberComplexity.l_two_mul_eq_add_one_of_binaryWeight_le_two
