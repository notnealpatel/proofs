...
/-!
# Asymptotic full Boolean row rank

This challenge concerns square matrices over the two-element Boolean semiring,
where addition is coordinatewise `or` and multiplication is `and`.  Its rank is
the least number of existing rows whose coordinatewise-`or` span equals the
span of every row.  It is not Boolean factorization/rectangle-cover rank and it
is not rank over `ZMod 2`.
-/
