...
This challenge asks for the exact number of finite-depth exponent sieve classes and for
Lagarias's counting bound. `exponentCount N` counts exponents `n < N`; its predicate is
stated literally using the base-3 digits of the value `2 ^ n`.
-/
...
digits zero and one is at most `2 * N ^ (log base 3 of 2)`. -/
